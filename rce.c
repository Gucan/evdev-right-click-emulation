...
int list_contains(const char *list, const char* name) {
	...
	int ret = list != NULL && strstr(list, fname) != NULL;
	free(fname);
	return ret;
}
...
int find_evdev(struct libevdev **devices) {
	...
	while((dev_input_entry = readdir(dev_input_fd)) != NULL) {
		...
		if(libevdev_has_event_type(evdev, EV_ABS) && !libevdev_has_event_type(evdev, EV_REL) && libevdev_has_event_code(evdev, EV_KEY, BTN_TOUCH) && !libevdev_has_event_code(evdev, EV_KEY, BTN_RIGHT)) {
